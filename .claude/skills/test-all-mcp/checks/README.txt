MCP smoke-test prompts -- one .txt per server, for upload-product-vue.
...
FILES (3 server checks)
  Docs:
    context7.txt         -> Vue/Vite/Tailwind/VeeValidate docs (remote HTTP, keyless) [turnkey]
  Browser:
    playwright.txt       -> browser automation (needs `just start` for :8102, or about:blank) [turnkey]
  Repo (per-dev):
    github.txt           -> GitHub identity + repo/PR (PAT) [per-dev]

PRECONDITIONS
  - Tool ids are mcp__<server>__<tool> (hyphens kept).
  - playwright(localhost) needs the dev server (`just start`, port 8102).
