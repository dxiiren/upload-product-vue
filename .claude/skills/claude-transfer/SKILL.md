...
3. **Re-ground before acting** (do NOT start new work first): read the brief's pointers + the
   recent `git log`, then run a **basic functional check** (e.g. `npx eslint .`, or `just start`
   and load http://localhost:8102) to catch drift or undocumented breakage. Building a new
...
- Files: `src/views/HomeView.vue:85-155`, `src/components/UploadProduct.vue`
- Recent commits: `<hash> <subject>` (run `git log --oneline -5`)
- Branch: {branch} PR: #{id}
- Commands: `just start`, `just lint`, `just build`
...
- Dev server on :8102 was {up/down} at close - re-check
- Backend on :8000 was {up/down} at close - table needs it for data
