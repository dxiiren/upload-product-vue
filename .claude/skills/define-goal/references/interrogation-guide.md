...
3. What proves **one** item is done? (Push for an evidence artifact - a pasted `just test` line,
   an eslint exit code, a screenshot path - not just a feeling.)
...
- What's the per-item verification (`npx eslint .` / `just test` / grep / a manual check on
  :8102) that proves the change is safe?
...
- This repo runs locally only (no CI/CD, no deploy target) - a "deployment" goal here usually
  means the local stack: `just build` green, `just preview` serving `dist/` on :8102, and the
  companion backend on :8000 reachable. State the health-verify that proves success.
