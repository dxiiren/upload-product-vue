...
- If there are multiple commits, synthesize one Conventional title that summarizes
  the change, e.g. `feat(products): add capacity column filter` or
  `fix(upload): reject non-xlsx files before submit`. Pick the type/scope from the
...
- <bullet per meaningful change — component/view/composable touched>
- <...>

## Testing

- <how it was verified: `just lint` result, `just test` result if specs exist,
  manual check against the dev server on :8102 (with/without the :8000 backend)>
```
...
| `/pre-pr-review` | Optional self-review (Vue/Tailwind/a11y) before opening the PR                |
